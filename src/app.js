(function () {
  ...
  function renderStartupError(message) {
    ...
    document.getElementById("pageTools")?.replaceChildren();
    app.innerHTML = `<section class="auth-card panel"><img class="auth-logo" src="assets/qassim-health-cluster-logo.png" alt="شعار تجمع القصيم الصحي"><h2>تعذر تشغيل النسخة السحابية</h2><p>${esc(message)}</p><button class="primary" id="retryStartup">إعادة المحاولة</button><p class="muted">يلزم ضبط DATABASE_URL وAPP_PASSWORD وSESSION_SECRET ثم إعادة النشر.</p></section>`;
  ...
  async function start() {
    ...
    document.getElementById("pageTools")?.replaceChildren();
    app.innerHTML = `<section class="auth-card panel"><img class="auth-logo" src="assets/qassim-health-cluster-logo.png" alt="شعار تجمع القصيم الصحي"><h2>جارٍ تحميل البيانات…</h2><p>لحظات ويتم التحقق من التخزين الآمن.</p></section>`;
    let result;
  ...
  function reportHeader(period){return `<section class="print-heading"><img class="report-logo" src="assets/qassim-health-cluster-logo.png" alt="شعار تجمع القصيم الصحي"><div><h2>متابعة الحالات الحرجة والوفاة الدماغية</h2><p>تجمع القصيم الصحي · تنسيق التبرع بالأعضاء</p></div><strong>${period}</strong></section>`;}
